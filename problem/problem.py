...
class Problem:
    ...
    def parse(self, args):
        ...
        if contest_data["choice"]:
            for version in contest_data["versions"]:
                ...
                if new:
                    self.version = random.choices(population=[version for version in contest_data["versions"] if contest_data[version]["new"]], weights=[
                                                  contest_data[version]["weight"] for version in contest_data["versions"] if contest_data[version]["new"]])[0]
                else:
                    self.version = random.choices(population=contest_data["versions"], weights=[
                                                  contest_data[version]["weight"] for version in contest_data["versions"]])[0]
        ...
        if new and not chosen:
            self.year = random.choice(
                [year for year in contest_data[self.version]["years"] if year >= 2010])
        else:
        ...
        for key in difficulties:
            if key in args:
                ...
                self.number = random.choice(
                    contest_data[self.version][self.difficulty])
                break
        else:
    ...
    def rating_updates(self):
        ...
        if contest_data["rated"]:
            if self.attempts > 0:
                update_list[contest_data["rating_name"]+"_RATING"] = contest_data["difficulty_weight"][["easy",
                                                                                                        "medium", "hard", "insane"].index(self.difficulty)]*contest_data["attempt_weight"][self.attempts-1]/10
                update_list["TOTAL_RATING"] = update_list[contest_data["rating_name"] +
                                                          "_RATING"]*contest_data["contest_weight"]/10
        return update_list
    ...
    def make_path(self):
        if self.version.startswith("None"):
            ...
            self.path = "{}/{}{}/{}".format(self.year,
                                            self.contest, self.version, self.number)
        else:
            self.path = "{}/{}/{}/{}".format(self.year,
                                             self.contest, self.version, self.number)
    ...
    def get_data(self):
        ...
        if contest_data["url"] is not None:
            if self.version.startswith("None"):
                self.urls["wiki"] = contest_data["url"].replace("YEAR", str(self.year)).replace(
                    "NUMBER", str(self.number)).replace("VERSION", "").replace("__", "_")
            else:
                self.urls["wiki"] = contest_data["url"].replace("YEAR", str(self.year)).replace(
                    "NUMBER", str(self.number)).replace("VERSION", self.version)
        else:
            self.urls["wiki"] = None

    def get_image(self):
        try:
            return discord.File("data/problems/{}/statement.png".format(self.path))
        except:
