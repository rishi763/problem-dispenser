...
from utils.discord_utils import get_name
import discord

async def leaderboard(args, author):
    ...
    for i in range(10*(number-1),min(10*number,len(leaderboard))):
        format_leaderboard+=("`"+str(i+1)+"."+"` "+str(get_name(leaderboard[i][0]))+" : "+str(leaderboard[i][1])+"\n").format(message)
    if leaderboard!=-1:
        if name==1:
    ...
    return [{"text":"There was an error while displaying the leaderboard, please contact <@!513126047978356736> about this."}]
