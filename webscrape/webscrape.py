...
def get_html(url, skips=[]):
    ...
    for item in elements:
        if len(item.find_elements(By.CLASS_NAME, "cmty-view-post-item-label")) == 0:
            ...
            try:
                int(item.find_element(By.CLASS_NAME,
                    "cmty-view-post-item-label").get_attribute("innerHTML"))
                if problems == []:
                    version = 0
                    problems.append([""])
                url = item.find_element(By.CLASS_NAME, "cmty-view-post-poster").find_element(
                    By.CLASS_NAME, "cmty-view-post-topic-link").find_element(By.TAG_NAME, "a").get_attribute("href")
                latex, asy = to_latex(item.find_element(
                    By.CLASS_NAME, "cmty-view-post-item-text"))
                problems[version].append(
                    {"latex": latex, "asy": asy, "url": url})
...
def compile(latex_text, asy, path):
    ...
    if asy:
        os.system(
            "cd webscrape/latex & pdflatex -interaction=nonstopmode statement.tex")
        ...
        if update:
            try:
                ...
                os.replace("webscrape/latex/statement.png",
                           "webscrape/data/{}/statement.png".format(path))
        else:
            os.replace("webscrape/latex/statement.png",
                       "webscrape/data/{}/statement.png".format(path))
    else:
        ...
        if update:
            try:
                os.replace("webscrape/latex/statement.png",
                           "data/problems/{}/statement.png".format(path))
            except:
                print("Failed to update files.")
        else:
            os.replace("webscrape/latex/statement.png",
...
def get_problems(urls):
    ...
    for url in urls:
        ...
        [name, skips, categories] = contests["_".join(
            url.replace("_spring", "").split("_")[2:])]
        try:
