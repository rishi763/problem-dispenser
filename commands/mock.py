...
async def mock(args, message, client):
    ...
    problems = [0]*(len(contests[contest]["None"]["easy"])+len(contests[contest]
                    ["None"]["medium"])+len(contests[contest]["None"]["hard"]))
    ...
    subprocess.run(
        "pdflatex -interaction=nonstopmode -output-directory=latex/mock mock.tex".split(" "))
    for file in os.listdir("latex/mock"):
        if file.endswith(".asy"):
            subprocess.run("asy --cd=latex/mock {}".format(file))
    subprocess.run(
        "pdflatex -interaction=nonstopmode -output-directory=latex/mock mock.tex".split(" "))
    ...
    for i in range(len(problems)):
        links_text += "{}: {} {}\n".format(
            i+1, problems[i].urls["forum"], problems[i].urls["wiki"])
