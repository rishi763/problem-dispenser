...
async def contest(args, message, client):
    path=args[0].upper()
    if path.replace("/","").isalnum() and os.path.exists("data/problems/{}".format(path)) and os.path.isdir("data/problems/{}".format(path)) and os.path.isdir("data/problems/{}/{}".format(path, os.listdir("data/problems/{}".format(path))[0])) and os.path.isfile("data/problems/{}/{}/info.json".format(path, os.listdir("data/problems/{}".format(path))[0])):
        await message.channel.send("Preparing contest...")
        year=int(path[:4])
        contest=None
        ...
        subprocess.run("pdflatex -interaction=nonstopmode -output-directory=latex/contest contest.tex".split(" "))
        await message.channel.send("Here is your contest!", file=discord.File("latex/contest/contest.pdf")) 
        await message.channel.send("You will be sent answers(if applicable) and links to solutions in {} seconds".format(contests[contest]["time"]))
        ...
        if contests[contest]["answer"]:
            with open("latex/contest/answers.txt","w") as answers_file:
                ...
                answers_file.flush()
            files.append(discord.File("latex/contest/answers.txt"))
        with open("latex/contest/links.txt", "w") as links_file:
            ...
            links_file.flush()
        files.append(discord.File("latex/contest/links.txt"))
        return await message.author.send("Here are the solutions!", files=files)
    else:
