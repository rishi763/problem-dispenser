...
async def latex(args, message, client):
    args = " ".join(args)
    path = "latex/{}".format(message.author.id)
    try:
        os.mkdir(path)
    except OSError:
        pass
    ...
    try:
        subprocess.run(
            "pdflatex -interaction=nonstopmode -output-directory={} statement.tex".format(path).split(" "), timeout=10)
    except Exception as e:
        print(e)
        return await message.channel.send("Your LaTeX failed to compile properly.")
    #subprocess.run("gs -sDEVICE=png16m -dTextAlphaBits=4 -r600 -o {0}/statement.png {0}/statement.pdf".format(path).split(" "))
    subprocess.run(
        "gswin64c -sDEVICE=png16m -dTextAlphaBits=4 -r600 -o {0}/statement.png {0}/statement.pdf".format(path).split(" "))
