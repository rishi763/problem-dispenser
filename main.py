...
async def on_message(message):
    ...
    if message.author == client.user and len(message.embeds) == 1 and message.embeds[0].title == "Leaderboard":
        ...
        reaction_messages.append([message, ["leaderboard", message.embeds[0].fields[0].name.split()[
                                 0].lower(), int(message.embeds[0].fields[0].name.split()[-1][:-1].lower())]])
        return
    ...
    #     await command_handler(message.content[4:].lower().split(), message, client)
        ...
        if message.content[1:].lower().split()[0] == "latex":
            await command_handler(message.content[1:].split(), message, client)
        else:
            await command_handler(message.content[1:].lower().split(), message, client)
...
async def on_message_edit(before, after):
    ...
    if (type(after.content) == str) and len(after.content) > 1 and after.content[0] == "?":
        await command_handler(after.content[1:].split(), after, client)
...
async def on_reaction_add(reaction, user):
    ...
    if reaction.message in [x[0] for x in reaction_messages]:
        reaction_message = reaction_messages[[
            x[0] for x in reaction_messages].index(reaction.message)]
        if reaction_message[1][0] == "leaderboard":
            ...
            await command_handler(reaction_message[1]+[channel], reaction_message[0], client)
