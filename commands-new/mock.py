...
def mock(args, author):
    contest=Problem(" ".join(args).lower()).contest
    if contest not in ["AIME", "AMC/8", "AMC/10", "AMC/12", "USAJMO", "IMO"]:
        return [{"text":"You either forgot to mention a contest or the contest you chose is not supported at the moment."}]
    ...
    with open("latex/mock/links.txt", "w") as links_file:
        ...
        links_file.flush()
        return [{"text":"Here is your mock.", "file":"latex/mock/mock.pdf", "dm":True},{"text":"You will be dmed again in {} seconds with an answer key if applicable and links to solutions for each problem.".format(contests[contest]["time"]), "time-skip":contests[contest]["time"], "dm":True},{"text":answer_text, "file":"latex/mock/links.txt", "dm":True}]
